...
document
  .querySelectorAll(
    "a,button,.sidebar-nav-item,.budget-option,.pp-card,.proj-row-header,.project-row-header",
  )
  .forEach((el) => {
      ...
      texts.forEach((id, i) => {
        setTimeout(() => {
          ...
          if (el) {
            ...
            el.style.transition = `transform 0.6s cubic-bezier(0.55,0,1,0.45) ${i * 0.04}s`;
          }
        }, i * 60);
      });
    }, 400);
...
const PROJECTS_DATA = [
  {
    ...
    status: "completed",
    contrib:
      "Full frontend architecture, interactive map integration (Leaflet), apartment designer system (Konva.js), advanced filtering system, comparison feature, saved properties system, responsive UI design, UX optimization, modular JavaScript architecture",
    problem:
      "Needed a modern rental platform concept that combines property search, map-based exploration, and interactive design tools for users to visualize and compare living spaces",
    tools: [
      "HTML5",
      "CSS3",
      "JavaScript",
  ...
  },
  {
    ...
    status: "completed",
    contrib:
      "Landing page design, UI/UX, frontend development, Figma prototyping",
    problem:
      "Required a visually compelling crypto trading platform landing page that converts visitors",
    tools: ["HTML5", "CSS3", "Adobe Photoshop", "Figma"],
    site: "case-study.html?project=crypto-precision",
    github: "https://github.com/hossam43/crybtobrecison-project",
    caseStudy: true,
...

  {
    ...
    status: "completed",
    contrib:
      "Full frontend build, animated preloader, responsive design, SEO optimization, deployment",
    problem:
      "Needed a fast, modern online presence for a language academy serving students across Egypt",
  ...
  {
    title: "MallNav Digital Twin",
    cat: "Development",
    year: "2026",
    status: "in-progress",
    contrib:
      "3D indoor navigation system, GIS integration, A* routing engine, live navigation UI, Cesium globe integration",
    problem:
      "Indoor environments lack real-time navigation and geospatial integration, making it difficult to connect real-world coordinates with indoor movement",
    tools: [
      "HTML5",
      "CSS3",
      "JavaScript",
    ...
    site: "case-study.html?project=mallnav", // add deployed link
    github: "", // add repo link
    img: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=600&q=60&fit=crop",
    filters: ["all", "Development", "3D", "GIS"],
  },

  {
    title: "Medicine Market",
    cat: "Development",
    year: "2023",
    status: "completed",
    contrib:
      "E-commerce UI, payment gateway integration, user authentication, responsive design",
    problem:
      "Online marketplace for medical products requiring secure payment and smooth UX",
    tools: ["HTML5", "CSS3", "JavaScript", "Figma"],
    site: "case-study.html?project=medicine-market",
...

  {
    ...
    status: "in-progress",
    contrib:
      "Interactive 3D city map, geospatial data visualization, custom map layers",
    problem:
      "Municipality needed an interactive platform to visualize urban planning data spatially",
    tools: ["Mapbox GL", "React", "Three.js", "GeoJSON"],
    site: "case-study.html?project=gis-city-viewer",
    github: "#",
...

  {
    ...
    status: "in-progress",
    contrib:
      "Infrastructure visualization, real-time monitoring UI, cost optimization analysis",
    problem:
      "SaaS company needed visual overview of their multi-cloud architecture",
...

  {
    ...
    status: "completed",
    contrib:
      "Real estate landing page, brand identity, image optimization, UI/UX design",
    problem:
      "Real estate firm needed a premium online presence to attract high-end clients",
...

  {
    ...
    status: "completed",
    contrib:
      "Responsive website, user-friendly navigation, fast loading, modern aesthetic",
    problem:
      "Fitness center needed a digital platform to attract new members and showcase services",
  ...
  {
    title: "3D Portfolio Viz",
    cat: "3D Experience",
    year: "2024",
    status: "in-progress",
    contrib:
      "Custom WebGL scene, Three.js particle systems, GSAP scroll animations",
    problem:
      "Showcase immersive 3D capabilities for premium clients in creative industries",
...

  {
    ...
    status: "in-progress",
    contrib:
      "Spatial data pipeline, interactive choropleth maps, filtering system, data storytelling",
    problem:
      "NGO needed accessible visualization of demographic & environmental data across regions",
...
PROJECTS_DATA.forEach((p, i) => {
  ...
  card.style.cursor = "none"; // keep your custom cursor

  card.addEventListener("click", (e) => {
    // prevent conflict if user clicks on inner links
    if (e.target.closest("a")) return;

    if (p.site) {
      if (p.caseStudy) {
        window.location.href = p.site; // internal case study
      } else {
        window.open(p.site, "_blank"); // external demo
      }
    }
  });
...
document.querySelectorAll(".pp-filter-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll(".pp-filter-btn")
...
ppGrid.addEventListener("mouseleave", () =>
  imgReveal.classList.remove("visible"),
);
...
document.getElementById("form-submit-btn").addEventListener("click", () => {
  ...
  if (!service) {
    document.getElementById("fg-service").classList.add("has-error");
    valid = false;
  }
  if (!msg) {
    document.getElementById("fg-msg").classList.add("has-error");
    valid = false;
  }
